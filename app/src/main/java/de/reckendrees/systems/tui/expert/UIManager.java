...
public class UIManager implements OnTouchListener {
    ...
    private class CommandUpdate implements OnCommandUpdate{
        @Override
        public void update(String updateString, int updateTime, Runnable runnable){
            UIManager.this.updateText(Label.command1, Tuils.span(mContext, labelSizes[Label.command1.ordinal()], updateString));
...
//        scrolllllll
        ...
        labelSizes[Label.command1.ordinal()] = XMLPrefsManager.getInt(Ui.command1_size);
        ...
        show[Label.command1.ordinal()] = XMLPrefsManager.getBoolean(Ui.show_command1);
        ...
        indexes[Label.command1.ordinal()] = show[Label.command1.ordinal()] ? XMLPrefsManager.getFloat(Ui.command1_index) : Integer.MAX_VALUE;
...
//                -1 = left     0 = center     1 = right
        ...
        if(show[Label.command1.ordinal()]) {
...
//    Pattern timePattern = Pattern.compile("(%t\\d*)(?:\\((?:(\\d+)([^\\)]*))\\)|\\((?:([^\\)]*)(\\d+))\\))?");
    ...
    private class getCommandOutputTask extends AsyncTask<String, Integer, String> {
        ...
        protected void onPostExecute(String output) {
            Log.d("TUI-E",output);
            commandUpdate.update(output,XMLPrefsManager.getInt(Ui.custom_command_timeout),runnable);
        }
    }
}
